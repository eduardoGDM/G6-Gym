    ...
    <Card className="border-border/80 bg-card/80">
      ...
      {isLoading ? (
        <CardContent className="overflow-x-auto p-0">
          <TableSkeleton
            columns={[
              ...
              { label: "Personal Responsável", className: "hidden md:table-cell" },
              "Status",
        ...
        <CardContent className="flex flex-col items-center justify-center gap-3 py-16 text-center animate-in fade-in duration-300">
          ...
          <h2 className="text-lg font-semibold">Nenhum aluno encontrado</h2>
        </CardContent>
      ) : (
        <>
          <CardContent
            ...
            <table className="min-w-full divide-y divide-border/70">
              ...
              <tbody className="divide-y divide-border/60 bg-card/70">
                {students.map((student) => (
                  <tr key={student.id} className="transition-colors hover:bg-muted/10">
                    ...
                    <td className="hidden px-4 py-3 text-sm text-muted-foreground md:table-cell">
                      {formatDateBR(student.created_at)}
                    </td>
                    <td className="px-4 py-3">
                      <StatusSwitch
                        checked={student.is_active}
                        confirmTitle={
                          student.is_active ? "Desativar aluno?" : "Ativar aluno?"
                        }
